...
function showHelp() {
    console.log(`
Usage: node scripts/create-test-tournament.js [options]
...
Exemples:
  node scripts/create-test-tournament.js
  node scripts/create-test-tournament.js --name "Mon Tournoi" --teams 16
  node scripts/create-test-tournament.js --type king --with-players 20
  npm run test-tournament -- --type elimination --teams 8
`);
}
 ...
 */
function generateRegistrationDates(timing, eventDate) {
    const now = new Date();
 ...
 */
async function createTestTournament(options) {
    ...
    try {
        ...
        console.log(`   Inscriptions: ${registrationDates.start.toLocaleDateString('fr-FR')} -> ${registrationDates.end.toLocaleDateString('fr-FR')}`);
        ...
        if (withPlayers > 0) {
            ...
            try {
                const createDummyPlayers = require('./create-dummy-players');
                await createDummyPlayers.createDummyPlayers({
...
module.exports = { createTestTournament };
